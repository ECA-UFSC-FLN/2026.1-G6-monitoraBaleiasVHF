...
# Mostra a senoide
onda1 = np.abs(sinal_A1[:250])

plt.subplot(2, 1, 1)
plt.plot(onda1, color='red', label='Sinal A1')
...
# plt.subplot(2, 1, 1)
# plt.plot(sinal_A2, color='blue', label='Sinal A2')
# plt.legend()

# 2. Sinal no Domínio da Frequência (Espectro de Potência)
plt.subplot(2, 1, 2)
plt.psd(sinal_A1, NFFT=N_FFT, Fs=sample_rate/1e6, Fc=freq_sinal/1e6)  # Convertendo para MHz
